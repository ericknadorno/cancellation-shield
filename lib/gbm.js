...
export function predictGBMProb(reservation, modelJson) {
  ...
  // Pass cohort_rates from the model JSON so the channelMonthRate feature
  // resolves against the same lookup table the trainer saw. Old models
  // (trained before Phase 1) don't have this field and that's fine —
  // scoreChannelMonthRate falls back to a global prior when the table is
  // absent, and the feature simply isn't in the tree's split history so
  // the value never matters.
  const fv = extractFeatureVector(reservation, modelJson.cohort_rates);
