...
// the training matrix produced the AUC 0.9587 false positive we saw in prod:
//
...
//
// Scoring (api/score.js, lib/gbm.js) still uses the full 30-feature vector so
...
export const LEAK_SAFE_FEATURE_NAMES = FEATURE_NAMES.filter(
  name => name !== "pay" && name !== "mod" && name !== "modCount"
       && name !== "adr" && name !== "adrVsAvg" && name !== "card"
);
...
export const scoreDOW = a => {
  if (!a) return 0;
  // "Z" + getUTCDay() keeps this deterministic across server timezones
  // (matches Python's naive-date .weekday()).
  const d = new Date(a + "T12:00:00Z").getUTCDay();
  if (d === 5 || d === 6) return 5;
  if (d === 0) return 3;
  return 0;
};
...
export const scoreShortMidweek = (nights, arrival) => {
  ...
  try {
    // "Z" + getUTCDay() keeps this deterministic across server timezones
    // (matches Python's naive-date .weekday()).
    const dow = new Date(arrival + "T12:00:00Z").getUTCDay(); // 0=Sun..6=Sat
